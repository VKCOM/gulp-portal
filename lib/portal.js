...
function deserialize(message) {
  ...
  file._contents = new Buffer.from(file._contents);
...
function sendToWorker(workerId, message, timeout, name) {
  ...
  return new Promise((resolve) => {
    ...
    timeoutId = setTimeout(() => {
      resolve({
        type: 'error',
        error: new PluginError('gulp-portal',
    ...
    jobs.push(sendToWorker(workerId, serialize(file), opts.timeout, file.path)
      .then((result) => {
        ...
        if (result.type === 'file') {
          ...
          callback(new PluginError('gulp-portal', result.error));
        }
      }).catch((err) => {
        callback(err);
      }));
